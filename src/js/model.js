...
const createRecipeObject = function (responseData) {
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const searchDataResponse = await AJAX(
      `${API_URL}?search=${query}&key=${KEY}`
    );
    // console.log(searchDataResponse);
    state.search.results = searchDataResponse.data.recipes.map(rec => {
      return {
        id: rec.id,
        image: rec.image_url,
...
export const uploadRecipe = async function (newRecipe) {
  try {
    ...
    const responseData = await AJAX(`${API_URL}?key=${KEY}`, recipe);
    state.recipe = createRecipeObject(responseData);
